...
def replay_case(row: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="parallel-letter-frequency-gap-") as temporary:
...
def main() -> None:
    ...
    if sha256(FIXTURE / "parallel_letter_frequency_test.cpp") != manifest["official"]["test_sha256"]:
