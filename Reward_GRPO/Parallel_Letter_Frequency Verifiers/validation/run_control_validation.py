...
FIXTURE = (
    ...
    / "benchmark-output-shard-1/cpp/exercises/practice/parallel-letter-frequency"
)
...
def reference_sources() -> dict[str, str]:
    return {
        "parallel_letter_frequency.h": (FIXTURE / ".meta/example.h").read_text(
            encoding="utf-8"
        ),
        "parallel_letter_frequency.cpp": (FIXTURE / ".meta/example.cpp").read_text(
            encoding="utf-8"
        ),
    }
...
def digit_counting_mutant() -> dict[str, str]:
    sources = reference_sources()
    old = "if (std::isalpha(c)) {"
    new = "if (std::isalnum(c)) {"
    if sources["parallel_letter_frequency.cpp"].count(old) != 1:
        raise RuntimeError("reference character filter drifted")
    sources["parallel_letter_frequency.cpp"] = sources[
        "parallel_letter_frequency.cpp"
    ].replace(old, new, 1)
    return sources


def lost_aggregation_mutant() -> dict[str, str]:
    sources = reference_sources()
    old = "totals[c] += count;"
    new = "totals[c] = count;"
    if sources["parallel_letter_frequency.cpp"].count(old) != 1:
        raise RuntimeError("reference aggregation statement drifted")
    sources["parallel_letter_frequency.cpp"] = sources[
        "parallel_letter_frequency.cpp"
    ].replace(old, new, 1)
    return sources
...
def candidate_case(
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"parallel-letter-frequency-{case_id}-"
    ) as temporary:
...
def invalid_case(case_id: str, kind: str, output_root: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(
        ...
        if kind == "tampered_official_test":
            with (exercise / "parallel_letter_frequency_test.cpp").open(
                "a", encoding="utf-8"
            ) as stream:
                stream.write("\n// validation tamper\n")
        elif kind == "missing_compiler":
            compiler = "parallel-letter-frequency-validator-compiler-does-not-exist"
        else:
            raise RuntimeError(f"unknown invalid case: {kind}")
        result = run_verifier(
            3, exercise, output_root / "invalid_cases" / case_id, compiler
        )
...
def main() -> None:
    ...
    case_sources = {
        ...
        "trial_2_missing_dependencies": saved_sources(
            "midband_rl_v2_trial_2_turn_1"
        ),
        "trial_2_partial_repair": saved_sources("midband_rl_v2_trial_2_final"),
        "trial_3_missing_cctype_and_partition_fault": saved_sources(
            "midband_rl_v2_trial_3_turn_1"
        ),
        "trial_3_runtime_partition_fault": saved_sources(
            "midband_rl_v2_trial_3_final"
        ),
        "digit_counting_mutant": digit_counting_mutant(),
        "lost_aggregation_mutant": lost_aggregation_mutant(),
    ...
    invalid = [
        invalid_case(
            "tampered_official_test", "tampered_official_test", args.output_dir
        ),
    ...
    for case_id in (
        ...
        statuses = {
            policy: row["status"]
            for policy, row in by_id[case_id]["policies"].items()
        }
        if set(statuses.values()) != {"pass"}:
            raise RuntimeError(f"valid implementation was restricted: {case_id} {statuses}")
    for case_id in (
        "trial_2_missing_dependencies",
        "trial_2_partial_repair",
        "trial_3_missing_cctype_and_partition_fault",
        "trial_3_runtime_partition_fault",
        "digit_counting_mutant",
        "lost_aggregation_mutant",
    ):
        ...
        if all(
            policies[policy]["status"] == "pass"
            for policy in ("E01", "E02", "E04", "E05")
        ):
    ...
    payload = {
        "schema_version": 1,
        "task_id": "local-aider-cpp/parallel-letter-frequency",
        "compiler": compiler,
