...
SOURCE_OVERLAY_INSTRUCTIONS = VALIDATION_ROOT / "fixed/source_overlay_instructions.md"
...
def terminal_gate(policies: dict[str, dict[str, Any]]) -> str:
    statuses = (policies["E05"]["status"], policies["E06"]["status"])
    if "invalid" in statuses:
        return "invalid"
    return "pass" if statuses == ("pass", "pass") else "fail"
...
def replay_case(row: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="grade-school-gap-") as temporary:
        exercise = Path(temporary) / "grade-school"
        ...
        policies = {
            f"E{number:02d}": run_verifier(
                number,
                exercise,
                output_root / "cases" / row["case_id"] / f"E{number:02d}",
            )
        ...
        if any(policy["status"] == "invalid" for policy in policies.values()):
            raise RuntimeError(f"evaluator invalid during replay: {row['case_id']} {policies}")
        gate_status = terminal_gate(policies)
        classification = classify(row["source_outcome"], gate_status)
        ...
        return {
            ...
            "policies": policies,
            "terminal_gate_status": gate_status,
...
def main() -> None:
    ...
    for policy_id, expected in manifest["current_verifier_sha256"].items():
        if policy_id == "shared_helper":
            observed = sha256(VERIFIER_ROOT / "_grade_school_common.py")
        else:
            observed = sha256(verifier(int(policy_id[1:])))
        if observed != expected:
            raise SystemExit(f"verifier digest mismatch: {policy_id}")
    if sha256(FIXTURE / "grade_school_test.cpp") != manifest["official"]["test_sha256"]:
        raise SystemExit("official test digest mismatch")
    if sha256(PINNED_INSTRUCTIONS) != manifest["official"]["verifier_contract_instructions_sha256"]:
        raise SystemExit("verifier contract instructions digest mismatch")
    if sha256(SOURCE_OVERLAY_INSTRUCTIONS) != manifest["evaluation_controls"]["source_overlay_instructions_sha256"]:
        raise SystemExit("source overlay instructions digest mismatch")
    ...
    payload = {
        ...
        "verifier_contract_instructions_sha256": sha256(PINNED_INSTRUCTIONS),
        "source_overlay_instructions_sha256": sha256(SOURCE_OVERLAY_INSTRUCTIONS),
        "static_policies": [f"E{number:02d}" for number in STATIC_POLICIES],
        "terminal_gate": ["E05", "E06"],
