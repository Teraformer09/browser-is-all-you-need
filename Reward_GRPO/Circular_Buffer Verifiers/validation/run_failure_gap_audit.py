...
FIXTURE = (
    ...
    / "benchmark-output-shard-0/cpp/exercises/practice/circular-buffer"
)
...
def run_verifier(script: Path, exercise: Path, output: Path) -> dict[str, Any]:
    environment = dict(os.environ)
    shared = str(REPO_ROOT / "Reward_GRPO/_shared")
    environment["PYTHONPATH"] = shared + os.pathsep + environment.get("PYTHONPATH", "")
    ...
    if not receipt_path.is_file():
        raise RuntimeError(
            f"verifier did not write a receipt: rc={process.returncode}; "
...
def classification(source_outcome: str, gate_status: str) -> str:
...
def replay_case(row: dict[str, Any], output_root: Path, baseline_script: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="circular-buffer-gap-") as temporary:
        ...
        baseline = run_verifier(
            baseline_script,
            exercise,
            output_root / "cases" / row["case_id"] / "baseline_E03",
        )
        current = {
            f"E{number:02d}": run_verifier(
                verifier(number),
                exercise,
                output_root / "cases" / row["case_id"] / f"current_E{number:02d}",
            )
            for number in range(1, 6)
        }
        after = {name: sha256(exercise / name) for name in SOURCE_FILES}
        baseline_classification = classification(row["source_outcome"], baseline["status"])
        current_classification = classification(row["source_outcome"], current["E03"]["status"])
        if baseline_classification != row["expected_baseline_classification"]:
            raise RuntimeError(
                f"baseline classification drift for {row['case_id']}: "
                f"expected {row['expected_baseline_classification']}, observed {baseline_classification}"
            )
        if current_classification != row["expected_current_classification"]:
            raise RuntimeError(
                f"current classification drift for {row['case_id']}: "
                f"expected {row['expected_current_classification']}, observed {current_classification}"
            )
        return {
            "case_id": row["case_id"],
...
def main() -> None:
    ...
    if sha256(VALIDATION_ROOT / "baseline_contract.py") != manifest["baseline"]["contract_sha256"]:
        raise SystemExit("baseline contract digest mismatch")
    if sha256(VERIFIER_ROOT / "_contract.py") != manifest["current"]["contract_sha256"]:
        raise SystemExit("current contract digest mismatch")
    for policy_id, expected in manifest["current"]["verifier_sha256"].items():
        if sha256(verifier(int(policy_id[1:]))) != expected:
            raise SystemExit(f"current verifier digest mismatch: {policy_id}")
    if sha256(FIXTURE / "circular_buffer_test.cpp") != manifest["official"]["test_sha256"]:
        raise SystemExit("official test digest mismatch")

    with tempfile.TemporaryDirectory(prefix="circular-buffer-baseline-") as temporary:
        baseline_root = Path(temporary) / "verifiers"
        baseline_root.mkdir(parents=True)
        shutil.copy2(VALIDATION_ROOT / "baseline_contract.py", baseline_root / "_contract.py")
        baseline_script = baseline_root / verifier(3).name
        shutil.copy2(verifier(3), baseline_script)
        samples = [
            replay_case(row, args.output_dir, baseline_script)
            for row in manifest["cases"]
        ]

    if any(not row["source_unchanged"] for row in samples):
        raise RuntimeError("a verifier changed candidate source bytes")
    baseline_counts = Counter(row["baseline_classification"] for row in samples)
    current_counts = Counter(row["current_classification"] for row in samples)
    if baseline_counts.get("missed_failure") != 1:
        raise RuntimeError(f"expected one baseline missed failure, observed {baseline_counts}")
    if current_counts.get("missed_failure", 0) or current_counts.get("restriction", 0):
        raise RuntimeError(f"corrected contract still disagrees with source outcomes: {current_counts}")
    ...
    payload = {
        ...
        "fixture_sha256": {
            "circular_buffer_test.cpp": sha256(FIXTURE / "circular_buffer_test.cpp"),
        ...
        "baseline_contract_sha256": sha256(VALIDATION_ROOT / "baseline_contract.py"),
        "current_contract_sha256": sha256(VERIFIER_ROOT / "_contract.py"),
    ...
    print(json.dumps({
        ...
        "baseline": payload["baseline_classification_counts"],
        "current": payload["current_classification_counts"],
    }, sort_keys=True))
