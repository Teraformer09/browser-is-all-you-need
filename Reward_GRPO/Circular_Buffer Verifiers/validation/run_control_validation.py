...
def reference_sources() -> dict[str, str]:
    return {
        "circular_buffer.h": (FIXTURE / ".meta/example.h").read_text(encoding="utf-8"),
...
def overwrite_head_mutant() -> dict[str, str]:
    sources = reference_sources()
    old = "if (is_full()) move_position(head_);"
    new = "if (is_full()) { /* intentionally keep the stale head */ }"
    if sources["circular_buffer.h"].count(old) != 1:
        raise RuntimeError("reference overwrite statement drifted")
    sources["circular_buffer.h"] = sources["circular_buffer.h"].replace(old, new, 1)
    return sources
...
def candidate_case(case_id: str, sources: dict[str, str], output_root: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"circular-{case_id}-") as temporary:
...
def invalid_case(case_id: str, kind: str, output_root: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix=f"circular-{case_id}-") as temporary:
        ...
        if kind == "tampered_official_test":
            with (exercise / "circular_buffer_test.cpp").open("a", encoding="utf-8") as stream:
                stream.write("\n// validation tamper\n")
        elif kind == "missing_compiler":
            compiler = "circular-buffer-validator-compiler-does-not-exist"
        else:
...
def main() -> None:
    ...
    case_sources = {
        ...
        "midband_trial_2_turn_2_positive": saved_sources("midband_rl_v2_trial_2_turn_2"),
        "midband_trial_3_turn_1_positive": saved_sources("midband_rl_v2_trial_3_turn_1"),
        "head_tail_alias_failure": saved_sources("midband_rl_v2_trial_1_final_turn_2"),
        "duplicate_definitions_failure": saved_sources("midband_rl_v2_trial_2_turn_1"),
        "overwrite_count_failure": saved_sources("midband_rl_v2_trial_4_turn_1"),
        "overwrite_head_mutant": overwrite_head_mutant(),
    ...
    for case_id in (
        "reference_positive",
        "midband_trial_2_turn_2_positive",
        "midband_trial_3_turn_1_positive",
        "reference_repeat",
    ):
    ...
    for case_id in (
        "head_tail_alias_failure",
        "duplicate_definitions_failure",
        "overwrite_count_failure",
        "overwrite_head_mutant",
    ):
    ...
    payload = {
        "schema_version": 1,
        "task_id": "local-aider-cpp/circular-buffer",
        "compiler": compiler,
