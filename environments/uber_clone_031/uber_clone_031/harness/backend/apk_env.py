...
from uber_clone_031.harness.backend.core.observations import ObservationMode, build_observation
from uber_clone_031.harness.backend.core.safety import SafetyPolicy
from uber_clone_031.harness.backend.env import StepResult
from uber_clone_031.harness.backend.reset_manager import reset_task_device
...
class DummyApkEnv:
    ...
    def _coerce_action(self, raw_action: ApkAction | dict[str, Any] | str) -> ApkAction:
        if isinstance(raw_action, ApkAction):
            return raw_action
        # Check object shape before mapping operations for strings and decoded
        # objects alike; the parser also handles legacy action/target keys.
