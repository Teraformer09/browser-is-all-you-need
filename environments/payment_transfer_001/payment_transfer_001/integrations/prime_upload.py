"""Upload exactly one frozen payment model episode and verify every saved frame."""
import argparse
import base64
import json
import re
...
from payment_transfer_001.harness.evidence import digest, save
...
def prepare(root):
    ...
    if metadata.get("env_id")!="terrano09/payment-transfer-001":
...
def upload(root,env_id):
    ...
    if not receipt.get("evaluation_id"):
        created=client.create_evaluation(name="payment-transfer-001-"+root.name,
            environments=[{"id":env_id}],model_name=metadata["model"],dataset="payment_transfer_001",
            framework="payment-14x5-v1",task_type="android-ui",
            description="One real OpenRouter model rollout on a local-server KVM emulator; 14 policies, 70 evidence strategies and original per-action screenshots. Not Prime-hosted compute. Simulated payment only.",
