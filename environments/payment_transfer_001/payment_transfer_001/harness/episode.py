from payment_transfer_001.specs import task_path, fingerprints
from payment_transfer_001 import VERSION
import argparse
import base64
import json
import os
...
from payment_transfer_001.harness.device import Device
from payment_transfer_001.harness.evidence import save, digest
...
from payment_transfer_001.verification.generic.interaction import target_ok
...
class Episode:
    def __init__(self, serial, apk, output, adb="adb"):
        ...
        load_registry()
        self.episode = "payment_eval_" + uuid4().hex
    ...
    def step(self, raw):
        ...
        if not err and action["type"] == "finish" and not endpoint_test("T6", before["state"], self.task["expected"]):
            err = "Payment incomplete; finish rejected. Continue within the remaining budget."
        if err:
        ...
        self.done = bool(self.error or len(self.transitions) >= self.task["max_steps"] or
                         after.get("state",{}).get("status") == "COMPLETED")
        self.checkpoint()
        return after
