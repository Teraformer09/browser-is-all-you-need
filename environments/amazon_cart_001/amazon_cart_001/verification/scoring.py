...
POLICY_IDS = ("V1", "V2", "G1", "G2", "G3", "G4", "S1", "T1", "T2", "T3", "T4", "T5", "T6", "T7")
...
def score_episode(policy_results):
    if not isinstance(policy_results, list) or len(policy_results) != 14 or {
        p.get("policy_id") for p in policy_results if isinstance(p, dict)
    } != set(POLICY_IDS):
        return {"status": "INVALID", "reward": 0, "training_eligible": False,
                "reason_codes": ["EXPECTED_FOURTEEN_DISTINCT_POLICIES"], "policies": policy_results}
    ...
    return {"status": status, "reward": REWARDS[status], "training_eligible": status != "INVALID",
            ...
            "policies": policy_results, "policy_count": 14, "verifier_count": 70,
