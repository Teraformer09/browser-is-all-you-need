...
public final class StateStore {
    ...
    public StateStore(Context context) { preferences = context.getSharedPreferences("shopping_state", Context.MODE_PRIVATE); }
    ...
    public JSONObject save(AppState state, String action, boolean accepted, String error) throws Exception {
        ...
        if (!preferences.edit().putString("snapshot", snapshot.toString()).commit())
            throw new IllegalStateException("Shopping state could not be persisted");
        events = next;
        return snapshot;
    }
