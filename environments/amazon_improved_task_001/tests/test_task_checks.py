...
import unittest
from amazon_improved_task_001.verification.records import verify
from amazon_improved_task_001.verification.progress import weighted_stage_result

TASK=json.loads((Path(__file__).parents[1]/"amazon_improved_task_001/specs/task.json").read_text())
...
class VerifierTests(unittest.TestCase):
    def test_correct_three_item_cart(self):
        ...
        self.assertEqual(r["stage_result"]["value"],1.3)
        self.assertEqual(r["final_result"],1.3)
    ...
    def test_weighted_stage_result_partial_progress(self):
        stages=[{"id":"search_a","complete":True},{"id":"search_b","complete":True},{"id":"search_c","complete":False},{"id":"cart_a","complete":True},{"id":"cart_b","complete":True},{"id":"cart_c","complete":True}]
        result=weighted_stage_result(stages)
        self.assertEqual(result["passed_search_stages"],2)
        self.assertEqual(result["passed_cart_stages"],3)
        self.assertAlmostEqual(result["value"],1.1333333333)

    def test_weighted_stage_result_empty(self):
        self.assertEqual(weighted_stage_result([])["value"],0.0)
