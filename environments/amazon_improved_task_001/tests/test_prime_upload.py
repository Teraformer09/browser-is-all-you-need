...
from amazon_improved_task_001.integrations.legacy_scripted import digest, image_hashes, inside, prepare, verify_remote
...
class UploadResultTests(unittest.TestCase):
    ...
    def metadata(self):
        return {"task_id": "amazon_improved_task_001", "policy": "scripted-ui-validation",
    ...
    def test_model_run_not_accepted_by_scripted_exporter(self):
        meta = self.metadata()
        meta["model_calls"] = 1
        with patch("amazon_improved_task_001.integrations.legacy_scripted.read_json", return_value=meta):
