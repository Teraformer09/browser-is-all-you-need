...
from amazon_improved_task_001.harness.actions import PACKAGE, TARGETS, decode, schema_error, permission_error
from amazon_improved_task_001.harness.evidence import digest, nodes
from amazon_improved_task_001.specs import task_path
from amazon_improved_task_001.verification.registry import evaluate, load_registry, VERIFIERS
from amazon_improved_task_001.verification.evidence_readers import replay_operations
from amazon_improved_task_001.verification.scoring import score_policy, REWARDS
from amazon_improved_task_001.verification.task_checks import cart_complete
...
class PolicyTests(unittest.TestCase):
    ...
    def test_full_cart_has_fifteen_passing_policies(self):
    ...
    def test_registry_has_seventy_distinct_functions(self):
        self.assertEqual(len(load_registry()["policies"]), 15)
        self.assertEqual(len(VERIFIERS), 75)
        self.assertEqual(len({id(v[1]) for v in VERIFIERS.values()}), 75)
