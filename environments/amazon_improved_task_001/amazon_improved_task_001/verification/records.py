...
def _stage_result(stages):
    return weighted_stage_result([{"id":k,"complete":v.get("completed",False)} for k,v in stages.items()])
...
def result(status,reason,checks=None,stages=None):
    ...
    return {"status":status,"reward":{"PASS":1,"FAIL":-1,"INVALID":0}[status],
            ...
            "stage_result":_stage_result(stages),"final_result":_stage_result(stages)["value"],
