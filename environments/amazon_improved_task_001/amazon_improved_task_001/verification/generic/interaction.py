from amazon_improved_task_001.verification.generic.validity import validity_check
from amazon_improved_task_001.verification.semantic import semantic_error
from amazon_improved_task_001.verification.contracts import Unassessable, require
from amazon_improved_task_001.verification.evidence_readers import artifact, state
from amazon_improved_task_001.verification.task_checks import endpoint_test
import copy
import json
import re
import xml.etree.ElementTree as ET
from amazon_improved_task_001.harness.actions import PACKAGE, decode, schema_error, permission_error
from amazon_improved_task_001.harness.evidence import nodes
from amazon_improved_task_001.verification.records import verify as record_verify
...
def generic_check(c, policy, method):
    ...
    for i, (action, transition) in enumerate(zip(source, transitions)):
        ...
        if policy == "G2" and permission_error(action):
            return False
        if policy == "V15":
            # Actor discipline: every rejected action, identical consecutive
            # actions, repeated finish attempts and focus taps immediately
            # before typing are discipline violations. Diagnostic only.
            if not receipt["executed"] or receipt["accepted"] is False:
                return False
            if i and source[i - 1] == action:
                return False
            if action["type"] == "finish" and any(s.get("type") == "finish" for s in source[:i]):
                return False
            if (action["type"] == "type_text" and i and
                    source[i - 1] == {"type": "tap_element", "element_id": action.get("element_id")}):
                return False
        if policy == "G3":
        ...
        if policy == "S1" and semantic_error(action, c["task"]):
            return False
        if policy == "T7":
            ...
            if any(e["accepted"] is False for e in after["state"]["events"]):
                return False
            if action["type"] == "finish" and not endpoint_test("T6", current, c["task"]):
                return False
