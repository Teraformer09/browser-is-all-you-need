...
from amazon_improved_task_001.verification.evidence_readers import state
from amazon_improved_task_001.verification.task_checks import endpoint_test, visual_test
...
from amazon_improved_task_001.harness.actions import PACKAGE
from amazon_improved_task_001.verification.scoring import POLICY_IDS, REWARDS, score_policy, score_episode, invalid_result
...
def load_registry():
    ...
    require(data["scoring"] == {"status_to_reward": REWARDS, "required_policy_count": 15,
        ...
        try:
            if policy in {"T1","T2","T3","T4","T5","T6"}:
                ...
                if method in {"accessibility","screenshot_ocr"}:
                    ...
                    passed = endpoint_test(policy, actual, context["task"])
...
def evaluate(context):
    ...
    result["scoring_version"] = "cart_14x5_v1"
