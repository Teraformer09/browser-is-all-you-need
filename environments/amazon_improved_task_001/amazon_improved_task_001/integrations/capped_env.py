...
from amazon_improved_task_001.integrations.campaign_budget import (CampaignBudget, campaign_plan, bounded_prompt,
...
class CappedHostedAmazonCartEnv(HostedAmazonCartEnv):
    def __init__(self, max_total_spend_usd=None, completed_attempts=0,
        ...
        for key, required in {"sandbox_image": "ubuntu:24.04", "acceleration": "software",
                              ...
                              "max_attempts": self.run_attempt_limit, "rubric_profile": "peach_action_budget_v1"}.items():
    ...
    async def setup_state(self, state):
        ...
        # A sandbox/Android startup failure invalidates only that rollout. Its
        # VM reservation remains charged, while later bounded attempts remain
        # available. Inference/context failures still fail closed below.
        if state.get("hosted_error"):
            state["info"]["campaign_setup_failure_isolated"] = True
            self.budget.persist()
        return result
