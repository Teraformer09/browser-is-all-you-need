"""Prime adapter: the same cart episode and signed rubric as the local runner."""
import os
import json
import verifiers as vf
from datasets import Dataset
from amazon_improved_task_001.harness.episode import Episode
from amazon_improved_task_001.harness.prompts import SYSTEM_PROMPT
from amazon_improved_task_001.specs import task_path
from amazon_improved_task_001.verification.scoring import REWARDS
...
class AmazonCart001Env(vf.MultiTurnEnv):
    def __init__(self, allow_eval=False, adb_serial=None, adb_path=None, apk_path=None,
                 artifact_dir="artifacts/amazon_improved_task_001", **kwargs):
        ...
        super().__init__(dataset=dataset,eval_dataset=dataset,max_turns=self.task["max_steps"],
                         env_id="amazon-improved-task-001",max_workers=1,system_prompt=SYSTEM_PROMPT,
...
def load_environment(execution="hosted", capped_campaign=False, **kwargs):
    """Default to the latest peach app on Prime; old ADB mode is explicit."""
    if execution == "hosted" and capped_campaign is True:
        from amazon_improved_task_001.integrations.capped_env import CappedHostedAmazonCartEnv
        return CappedHostedAmazonCartEnv(**kwargs)
    if execution == "hosted":
        from amazon_improved_task_001.integrations.hosted_env import HostedAmazonCartEnv
        return HostedAmazonCartEnv(**kwargs)
    if execution == "legacy_local_adb":
        return AmazonCart001Env(**kwargs)
    raise ValueError("execution must be hosted or legacy_local_adb")
