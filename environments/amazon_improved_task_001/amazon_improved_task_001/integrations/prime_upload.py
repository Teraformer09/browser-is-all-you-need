...
def prepare(root):
    ...
    if metadata.get("env_id")!="terrano09/amazon-improved-task-001":
...
def upload(root,env_id):
    ...
    if not receipt.get("evaluation_id"):
        created=client.create_evaluation(name="amazon-improved-task-001-"+root.name,
            environments=[{"id":env_id}],model_name=metadata["model"],dataset="amazon_improved_task_001",
