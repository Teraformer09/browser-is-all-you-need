...
from amazon_improved_task_001.harness.hosted_worker import MAX_ARCHIVE, MAX_RESPONSE, RUNTIME
...
MODULE = "amazon_improved_task_001.harness.hosted_worker"
...
class HostedSession:
    ...
    def start(self):
        ...
        if self.bootstrap_runtime:
            from amazon_improved_task_001.integrations.runtime_bootstrap import provision
            provision(self)
        ...
        self.persist()
        result = self.call("reset")
        # A launched process is not a usable Android session until reset has
        # completed. Cleanup must not finalize an uninitialized worker and mask
        # the original startup failure.
        self.started = True
        self.info["worker_initialized"] = True
        self.persist()
        return result

    def command_environment(self):
        if self.bootstrap_runtime:
            from amazon_improved_task_001.integrations.runtime_bootstrap import REMOTE_ENV
    ...
    def call(self, operation, action=None):
        ...
        elif operation == "model_evidence":
            request["model_evidence"] = action
    ...
    def live_frame(self):
        ...
        command = shlex.join(["python", "-m", "amazon_improved_task_001.harness.live_device",
        ...
        try:
            if self.started:
                ...
                if self.rubric_profile == "peach_action_budget_v1":
                    self.call("model_evidence", {"schema_version": 1, "source": "host_controller",
                        "episode_id": self.info.get("episode_id"),
                        "requests": self.info.get("model_evidence_requests", [])})
