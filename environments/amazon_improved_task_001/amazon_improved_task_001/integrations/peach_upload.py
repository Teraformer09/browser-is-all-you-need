...
from amazon_improved_task_001.harness.evidence import digest,save
from amazon_improved_task_001.integrations.prime_account import api_client
from amazon_improved_task_001.integrations.prime_upload import prepare,verify_remote,SECRET
...
def upload(root,env_id):
    ...
    if not receipt.get("evaluation_id"):
        created=client.create_evaluation(name="DemoCart-peach-model-recorded-"+root.name,
            environments=[{"id":env_id}],model_name=metadata["model"],dataset="amazon_improved_task_001-peach",
