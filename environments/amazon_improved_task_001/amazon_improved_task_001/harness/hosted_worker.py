...
def validate_request(request, index):
    ...
    keys = ({"sequence", "operation", "action"} if operation == "step" else
            {"sequence", "operation", "model_evidence"} if operation == "model_evidence" else {"sequence", "operation"})
    if set(request) != keys or operation not in {"reset", "step", "finalize", "model_evidence"}:
    ...
    if operation == "model_evidence" and (not isinstance(request["model_evidence"], dict)
            or len(json.dumps(request["model_evidence"])) > 1024 * 1024):
        raise ValueError("Controller evidence must be a bounded object")
...
class Worker:
    ...
    def handle(self, request):
        ...
        if operation == "model_evidence":
            if self.rubric_profile != "peach_action_budget_v1" or self.finalized is not None:
                raise RuntimeError("Controller evidence unavailable for this profile/state")
            self.env.set_model_evidence(request["model_evidence"])
            return {"received": True, "episode_id": self.env.episode}
...
def main():
    ...
    parser.add_argument("--rubric-profile", choices=["legacy_v1", "peach_strict_v1", "peach_action_budget_v1"], default="legacy_v1")
