...
from amazon_improved_task_001.harness.episode import Episode
from amazon_improved_task_001.harness.evidence import save,digest,stamp
from amazon_improved_task_001.harness.actions import decode,schema_error
from amazon_improved_task_001.harness.peach import PeachDevice,TextInputUnconfirmed,PACKAGE,PROFILE,permission_error,target_node,ui_nodes
from amazon_improved_task_001.verification.peach import task_spec,snapshot,acceptance,cart_complete,progress,evaluate
...
class PeachEpisode(Episode):
    def __init__(self,serial,apk,output,adb="adb",execution_location="local-server-kvm",rubric_profile="legacy_v1"):
        ...
        save(self.root/"provenance.json",{"execution_location":execution_location,"package":PACKAGE,
            ...
            "policy_count":3 if rubric_profile == "peach_action_budget_v1" else 14,
            "correctness_check_count":9 if rubric_profile == "peach_action_budget_v1" else None,
            "shared_optimizer_count":5 if rubric_profile == "peach_action_budget_v1" else None,
    ...
    def set_model_evidence(self, envelope):
        """Controller-only usage receipts; this is never an actor action."""
        if self.verdict is not None:
            raise RuntimeError("Cannot change evidence after finalization")
        if (not isinstance(envelope, dict) or envelope.get("episode_id") != self.episode
                or envelope.get("source") != "host_controller"
                or len(json.dumps(envelope)) > 1024 * 1024):
            raise ValueError("Malformed or wrong-episode controller evidence")
        self.context["model_evidence"] = envelope
        save(self.root / "model_evidence.json", envelope)

    def assess(self):
        if self.rubric_profile == "peach_action_budget_v1":
            from amazon_improved_task_001.verification.action_budget import evaluate as budget_evaluate
            return budget_evaluate(self.context)
        if self.rubric_profile == "peach_strict_v1":
            from amazon_improved_task_001.verification.strict import evaluate as strict_evaluate
    ...
    def checkpoint(self):
        ...
        point.update(step=len(self.transitions),policy_results=verdict["policies"],
            strict_checks=verdict.get("strict_checks",[]),shared_optimizers=verdict.get("shared_optimizers",[]),
            rubric_profile=self.rubric_profile,episode_reward=None,
    ...
    def finalize(self):
        self.context["actor_stopped"] = True
        self.context["pipeline_error"]=self.error
        ...
        if self.history:
            self.history[-1].update(episode_reward=self.verdict["reward"],episode_status=self.verdict["status"],
                ...
                shared_optimizers=self.verdict.get("shared_optimizers",[]))
            save(self.root/"progress_history.json",self.history)
        ...
        if self.rubric_profile == "peach_action_budget_v1":
            for policy in self.verdict["policies"]:
                save(self.root/"policy_receipts"/(policy["policy_id"]+".json"), policy)
            save(self.root/"shared_optimizer_receipts.json", self.verdict["shared_optimizers"])
        if self.frames:
