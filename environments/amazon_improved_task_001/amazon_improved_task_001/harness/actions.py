"""Strict, app-scoped JSON actions. No response repair or reference-action substitution."""
import json
import re

PACKAGE = "com.primeintellect.shoppingdemo"
SKUS = {"headphones_wireless_001", "headphones_wired_001", "bottle_steel_001",
        "bottle_sport_001", "backpack_laptop_001", "backpack_daypack_001"}
INPUTS = {"search_input"}
TARGETS = {"search_input", "search_button", "cart_button", "continue_shopping_button"} | {
    prefix + sku for prefix in ("add_", "increase_", "decrease_", "remove_") for sku in SKUS}
KINDS = {"tap_element", "type_text", "press_back", "swipe", "wait", "finish"}
SCHEMA = {"type": "object", "additionalProperties": False, "required": ["type"],
          "properties": {"type": {"enum": sorted(KINDS)}, "element_id": {"type": "string"},
                         "text": {"type": "string"}, **{k: {"type": "integer"}
...
def permission_error(a):
    ...
    if a["type"] == "type_text":
        ...
        if not re.fullmatch(r"[A-Za-z0-9 ._-]*", a["text"]):
            return "Unsupported input characters"
    return None
